...
static int count_num(long long int num, int negative)
{
    int ret = 0;
...
static char *add_nb(long long int num, int negative)
{
    int i = 0;
...
{
    char *ret = NULL;
    long long int nb = va_arg(c, const long long int);
    char *temp = NULL;
    ...
    temp = long_long_int_to_string(nb);
    if (temp == NULL)
        return NULL;
